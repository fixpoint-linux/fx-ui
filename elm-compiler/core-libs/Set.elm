...
-- elm/core 1.0.5 Set.elm port — VERBATIM except: import lines are stripped
-- (Dict.*/List.*/Maybe.* refs resolve through the merged global table and the
-- implicit prelude alias rows; Lower.Module.compileUnit) and the `()` unit
-- value/ctor already lower to the VM's unit symbol.  Everything else is
