...
-- elm/core 1.0.5 Maybe.elm port (VERBATIM functions, docs trimmed) with two
-- subset deviations:
--   * the `type Maybe a` declaration is NOT redeclared here — the Just /
--     Nothing constructors live in the injected Prelude (preludeTable aliases
--     the bare names here), so patterns and ctor refs keep working;
--   * import lines are stripped (the dotted Dict.*/List.* refs resolve through
