...
-- elm/core 1.0.5 Result.elm port (VERBATIM functions, docs trimmed) with two
-- subset deviations:
--   * the `type Result error value` declaration is NOT redeclared here — the
--     Ok / Err constructors live in the injected Prelude (preludeTable aliases
--     the bare names here), so patterns and ctor refs keep working;
--   * import lines are stripped (dotted Maybe.*/List.* refs resolve through
--     the merged global table, Lower.Module.compileUnit).
-- Everything else is character-identical to upstream elm/core 1.0.5.
