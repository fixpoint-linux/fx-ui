...
-- elm/core 1.0.5 Array.elm port (relaxed-radix balanced tree, VERBATIM
-- algorithm) with these subset deviations, everything else
-- character-identical to upstream:
--
--   * (D1) the five import lines are stripped (Bitwise./JsArray./Tuple./
--     List.* refs resolve through the merged global table,
--     Lower.Module.compileUnit; `List.drop`/`List.foldl`/`List.reverse`
--     resolve via the prelude table's dotted rows — Prelude.drop landed in
--     Wave 1, so D5's rewrite is a no-op and List.drop stays verbatim);
--   * (D2) `shiftStep = 5` literal (was `ceiling (logBase 2 (toFloat 32))`
--     — no float-log prims in the subset; constant for branchFactor 32);
--   * (D3) the three float-depth sites use integer arithmetic: the two
--     `floor (logBase branchFactor n)` depth computations become
--     `floorLog32` (bottom-level helper, exact for n >= 1; both call sites
--     guard with `max 1`), and treeFromBuilder's
--     `ceiling (nodeListSize / 32)` becomes `(nodeListSize + 31) // 32`;
--   * (D4) `remainderBy branchFactor len` becomes `len - tailIndex len`
--     (equal for len >= 0, the only call site: rem = n - (n//32)*32, and
--     tailIndex is exactly (n//32)*32 for non-negative n);
--   * (D6) the EIGHT self-referential let-helpers are hoisted to top level
--     with their captured function as a leading parameter (the subset's
--     let-functions cannot see their own name; mergeStepState precedent in
--     Dict): foldrNodeHelp, foldlNodeHelp, mapNodeHelp, indexedMapNodeHelp,
--     appendFoldTreeHelp, appendFoldBuilderHelp, sliceLeftLeafHelp,
--     builderCollectNodeHelp.  toIndexedList's `helper` is NOT
--     self-referential and stays in its let.
--
-- The Elm.JsArray kernel is substituted by core-libs/JsArray.elm (VM-vector
-- representation; identical op semantics — see that module's header).  The
-- opaque JsArray/Tree/Node types are NOT redeclared: annotations are
-- tolerated but never parsed (Dict precedent).
...
{-| Exact floor of log-base-32 for n >= 1 (D3) — integer replacement for the
subset's missing float-log prims.
-}
floorLog32 : Int -> Int
floorLog32 n =
    if n < branchFactor then
        0

    else
        1 + floorLog32 (n // branchFactor)
...
foldr func baseCase (Array_elm_builtin _ _ tree tail) =
    JsArray.foldr (foldrNodeHelp func) (JsArray.foldr func baseCase tail) tree


foldrNodeHelp : (a -> b -> b) -> Node a -> b -> b
foldrNodeHelp func node acc =
    case node of
        SubTree subTree ->
            JsArray.foldr (foldrNodeHelp func) acc subTree
...
foldl func baseCase (Array_elm_builtin _ _ tree tail) =
    JsArray.foldl func (JsArray.foldl (foldlNodeHelp func) baseCase tree) tail


foldlNodeHelp : (a -> b -> b) -> Node a -> b -> b
foldlNodeHelp func node acc =
    case node of
        SubTree subTree ->
            JsArray.foldl (foldlNodeHelp func) acc subTree
...
map func (Array_elm_builtin len startShift tree tail) =
    Array_elm_builtin
        len
        startShift
        (JsArray.map (mapNodeHelp func) tree)
        (JsArray.map func tail)


mapNodeHelp : (a -> b) -> Node a -> Node b
mapNodeHelp func node =
    case node of
        SubTree subTree ->
            SubTree <| JsArray.map (mapNodeHelp func) subTree
...
indexedMap func (Array_elm_builtin len _ tree tail) =
    ...
    in
        builderToArray True (JsArray.foldl (indexedMapNodeHelp func) initialBuilder tree)


indexedMapNodeHelp : (Int -> a -> b) -> Node a -> Builder a -> Builder a
indexedMapNodeHelp func node builder =
    case node of
        SubTree subTree ->
            JsArray.foldl (indexedMapNodeHelp func) builder subTree
...
append ((Array_elm_builtin _ _ _ aTail) as a) (Array_elm_builtin bLen _ bTree bTail) =
    ...
    if bLen <= (branchFactor * 4) then
        JsArray.foldl appendFoldTreeHelp a bTree
            |> appendHelpTree bTail
    else
        JsArray.foldl appendFoldBuilderHelp (builderFromArray a) bTree
            |> appendHelpBuilder bTail
            |> builderToArray True


appendFoldTreeHelp : Node a -> Array a -> Array a
appendFoldTreeHelp node array =
    case node of
        SubTree tree ->
            JsArray.foldl appendFoldTreeHelp array tree

        Leaf leaf ->
            appendHelpTree leaf array


appendFoldBuilderHelp : Node a -> Builder a -> Builder a
appendFoldBuilderHelp node builder =
    case node of
        SubTree tree ->
            JsArray.foldl appendFoldBuilderHelp builder tree
...
sliceLeft from ((Array_elm_builtin len _ tree tail) as array) =
    if from == 0 then
        ...
        let
            leafNodes =
                JsArray.foldr sliceLeftLeafHelp [ tail ] tree
...
sliceLeftLeafHelp : Node a -> List (JsArray a) -> List (JsArray a)
sliceLeftLeafHelp node acc =
    case node of
        SubTree subTree ->
            JsArray.foldr sliceLeftLeafHelp acc subTree
...
builderFromArray (Array_elm_builtin len _ tree tail) =
    { tail = tail
    , nodeList = JsArray.foldl builderCollectNodeHelp [] tree
    , nodeListSize = len // branchFactor
    }


builderCollectNodeHelp : Node a -> List (Node a) -> List (Node a)
builderCollectNodeHelp node acc =
    case node of
        SubTree subTree ->
            JsArray.foldl builderCollectNodeHelp acc subTree
