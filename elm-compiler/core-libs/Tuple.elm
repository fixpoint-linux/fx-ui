...
-- elm/core 1.0.5 Tuple.elm port (VERBATIM functions + docs) with these subset
-- deviations, everything else character-identical to upstream:
--
--   * import lines are stripped (upstream has none outside doc comments —
--     the `import String` lines live inside `{-| -}` blocks);
--   * the dotted Tuple.* refs resolve through the merged global table
--     (Lower.Module.compileUnit), like Dict/Set/Maybe/Result.
--
-- Tuple PATTERNS in argument position (`first (x,_) = x`) are subset-supported
-- (Dict.mergeStepState `(list, result)` precedent), so no case rewrite was
-- needed.  The (a,b) pair itself is the VM's 2-cell cons chain.
