...
-- elm/core 1.0.5 Dict.elm port (VERBATIM Okasaki R-B algorithm — insert/
-- balance + moveRedLeft/moveRedRight/removeMin delete) with these subset
-- deviations, everything else character-identical to upstream:
--
--   * import lines are stripped (Basics.compare/Maybe./List.* resolve through
--     the merged global table and the implicit prelude alias rows);
--   * EXACTLY THREE compare rewrites, because real Elm compiles `<` `>` on
--     comparables to compare-based codegen while this untyped subset lowers
--     `<` `>` to the VM's numeric-only prims — `compare` (Prelude.compare) is
--     the structural dispatcher, so it IS the faithful semantics:
--       (a) removeHelp:      `if targetKey < key then ... else ...`
--              -> `case compare targetKey key of LT -> ...; _ -> ...`
--       (b) merge/stepState: `if lKey < rKey then A else if lKey > rKey then B else C`
--              -> `case compare lKey rKey of LT -> A; GT -> B; EQ -> C`
--       (c) removeHelpEQGT's `targetKey == key` is KEPT (the `=` prim is
--           already structural).
--   * merge's RECURSIVE let-bound `stepState` is HOISTED to a top-level
--     helper `mergeStepState` (taking leftStep/bothStep/rightStep as leading
--     params): the subset's `let f x = ...` closures cannot see their own
--     name (lowerLetFunction lowers the body before the let slot exists), so
--     a self-recursive let-function is uncompilable; a top-level function
--     recurses through its Global.  Semantics identical (same key traversal).
--   * The driving `compare` is the runtime's structural order (UTF-8 byte
--     order for String/Char vs JS UTF-16 order — differs only for astral
--     chars; NaN compares EQ; ill-typed mixed comparisons fall to EQ instead
--     of erroring — the untyped subset has no type error to raise).
--
-- Signatures + docs are kept verbatim (tolerated, never parsed).  Recursion
-- depth is tree height (~2*log2 n = ~22 @ n=1000), so the non-tail
-- insertHelp/foldl/foldr nesting is safe on the ZINC VM.
...
merge leftStep bothStep rightStep leftDict rightDict initialResult =
  let
    (leftovers, intermediateResult) =
      foldl (mergeStepState leftStep bothStep rightStep) (toList leftDict, initialResult) rightDict
  in
    List.foldl (\(k,v) result -> leftStep k v result) intermediateResult leftovers


-- Upstream inlines this as a self-recursive let-function inside `merge`; the
-- subset cannot compile self-recursive let-functions (see header), so it is
-- hoisted here with the three steppers threaded through explicitly.
mergeStepState
  :  (comparable -> a -> result -> result)
  -> (comparable -> a -> b -> result -> result)
  -> (comparable -> b -> result -> result)
  ...
  case list of
    ...
    (lKey, lValue) :: rest ->
      case compare lKey rKey of
        LT ->
          mergeStepState leftStep bothStep rightStep rKey rValue (rest, leftStep lKey lValue result)
